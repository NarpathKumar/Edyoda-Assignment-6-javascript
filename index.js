...
function createCard(imgurl, alte, head, desc){
  ...
  var cardCon = document.createElement('div');
    cardCon.classList.add('cardContent')
    card.appendChild(cardCon);
  var hch = document.createElement('h1');
    hch.classList.add('cardHeader')
    hch.innerText = head;
    cardCon.appendChild(hch);
  var para = document.createElement('p');
    para.classList.add('cardText')
    para.innerText = desc;
    cardCon.appendChild(para);
...
for(var i=0; i<cards.length; i++){
  var card = createCard(cards[i].imgUrl, cards[i].imgAlt, cards[i].header, cards[i].description)
